package start;

import control.*;
import model.*;
import ui.*;
import util.*;

public class Online_Market_Util {
	public static User_Manager user_Manager=new User_Manager();
	public static Order_Manager order_Manager=new Order_Manager();
	public static Production_Manager production_Manager=new Production_Manager();
	public static Address_Manager address_Manager=new Address_Manager();
	public static Discount_Manager discount_Manager=new Discount_Manager();
	public static Comment_Manager comment_Manager=new Comment_Manager();
	public static ShopMenu_Manager shopMenu_manager=new ShopMenu_Manager();
}
